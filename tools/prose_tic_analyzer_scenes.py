#!/usr/bin/env python3
"""
Scene-Level Prose Tic Analyzer for The Wheel of Meat
=====================================================
Analyzes individual scene files (not compiled chapters) for prose tics:
...
Generates detailed revision proposals with line numbers for each scene file.
"""
...
class SceneAnalysis:
    ...
    def __init__(self, scene_name: str, file_path: Path):
...
class ChapterAnalysis:
    """Analysis results for a chapter (collection of scenes)."""
    def __init__(self, chapter_num: int, title: str, dir_name: str):
        self.chapter_num = chapter_num
        self.title = title
        self.dir_name = dir_name
        self.scenes = []

    def add_scene(self, scene: SceneAnalysis):
        self.scenes.append(scene)

    def total_tics(self) -> int:
        return sum(scene.total_tics() for scene in self.scenes)
    ...
    def tic_density(self) -> float:
        ...
        if total_words == 0:
            return 0.0
        return (self.total_tics() / total_words) * 1000
...
def analyze_scene(scene_path: Path) -> SceneAnalysis:
    """Analyze a single scene file for prose tics."""
    # Read the file
    ...
    analysis = SceneAnalysis(scene_name, scene_path)
...
def analyze_chapter(chapter_num: int, dir_name: str, title: str) -> Optional[ChapterAnalysis]:
    ...
    dir_path = CHAPTERS_DIR / dir_name / "scenes"

    if not dir_path.exists():
        print(f"  [SKIP] Chapter {chapter_num}: No scenes directory")
        return None

    # Find all scene markdown files
    scene_files = sorted(dir_path.glob("scene-*.md"))

    if not scene_files:
        print(f"  [SKIP] Chapter {chapter_num}: No scene files found")
        return None

    # Create chapter analysis
    chapter_analysis = ChapterAnalysis(chapter_num, title, dir_name)

    print(f"  [{chapter_num:02d}/26] {title} ({len(scene_files)} scenes)")

    for scene_file in scene_files:
        scene_analysis = analyze_scene(scene_file)
        chapter_analysis.add_scene(scene_analysis)

        # Print scene summary if it has tics
        if scene_analysis.total_tics() > 0:
            print(f"    - {scene_analysis.scene_name}: {scene_analysis.total_tics()} tics ({scene_analysis.tic_density():.1f} per 1K)")
...
def generate_markdown_report(analyses: List[ChapterAnalysis]) -> str:
    ...
    lines.append("# Prose Tic Analysis Report (Scene-Level)")
    lines.append("# The Wheel of Meat - Individual Scene Files")
    lines.append("")
    lines.append("---")
    lines.append("")

    # Overall summary
    total_tics = sum(ch.total_tics() for ch in analyses)
    total_words = sum(ch.total_words() for ch in analyses)
    total_scenes = sum(len(ch.scenes) for ch in analyses)
    avg_density = (total_tics / total_words * 1000) if total_words > 0 else 0

    lines.append("## Executive Summary")
    lines.append("")
    lines.append(f"- **Total chapters analyzed:** {len(analyses)}")
    lines.append(f"- **Total scenes analyzed:** {total_scenes}")
    ...
    for chapter in analyses:
        for scene in chapter.scenes:
            for tic_type, count in scene.tic_counts.items():
    ...
    # Top 15 scenes by density
    lines.append("### Scenes Requiring Most Attention (by tic density)")
    lines.append("")
    all_scenes = []
    for chapter in analyses:
        for scene in chapter.scenes:
            all_scenes.append((chapter.chapter_num, chapter.title, scene))

    sorted_scenes = sorted(all_scenes, key=lambda x: x[2].tic_density(), reverse=True)
    for i, (ch_num, ch_title, scene) in enumerate(sorted_scenes[:15], 1):
        lines.append(f"{i}. **Ch.{ch_num} ({ch_title})** - `{scene.scene_name}` - "
                    f"{scene.total_tics()} tics ({scene.tic_density():.1f} per 1K words)")
    lines.append("")
    lines.append("---")
    lines.append("")

    # Detailed chapter-by-chapter analysis
    lines.append("## Detailed Analysis by Chapter and Scene")
    lines.append("")

    for chapter in analyses:
        lines.append(f"## Chapter {chapter.chapter_num}: {chapter.title}")
        lines.append("")
        lines.append(f"**Total scenes:** {len(chapter.scenes)} | "
                    f"**Word count:** {chapter.total_words():,} | "
                    f"**Total tics:** {chapter.total_tics()} | "
                    f"**Density:** {chapter.tic_density():.2f} per 1K words")
        lines.append("")

        if chapter.total_tics() == 0:
        ...
        # Process each scene
        for scene in chapter.scenes:
            if scene.total_tics() == 0:
                continue

            lines.append(f"### Scene: `{scene.scene_name}`")
            lines.append("")
            lines.append(f"**File:** `{scene.file_path.relative_to(BASE_DIR)}`")
            lines.append(f"**Word count:** {scene.word_count:,} | **Tics:** {scene.total_tics()} | **Density:** {scene.tic_density():.2f}")
            lines.append("")

            # Group tics by type
            tics_by_type = defaultdict(list)
            for tic in scene.tics:
            ...
            for tic_type in sorted(tics_by_type.keys()):
                ...
                # List each instance
                for tic in tic_list[:30]:  # Limit to first 30 per type per scene
                    lines.append(f"**Line {tic.line_num}:**")
                    lines.append(f"> {tic.line_text[:200]}")
                    lines.append("")
                ...
                if len(tic_list) > 30:
                    lines.append(f"*...and {len(tic_list) - 30} more instances*")
                    lines.append("")
...
def generate_summary_report(analyses: List[ChapterAnalysis]) -> str:
    ...
    lines.append("=" * 90)
    lines.append("PROSE TIC ANALYSIS - SCENE-LEVEL SUMMARY")
    lines.append("=" * 90)
    lines.append("")
    ...
    for chapter in analyses:
        lines.append(f"CHAPTER {chapter.chapter_num}: {chapter.title}")
        lines.append("-" * 90)
        lines.append(f"{'Scene':<40} {'Words':>8} {'Tics':>6} {'Density':>8}")
        lines.append("-" * 90)

        for scene in chapter.scenes:
            lines.append(
                f"{scene.scene_name[:38]:<40} "
                f"{scene.word_count:>8,} "
                f"{scene.total_tics():>6} "
                f"{scene.tic_density():>8.2f}"
            )

        lines.append("-" * 90)
        lines.append(
            f"{'CHAPTER TOTAL':<40} "
            f"{chapter.total_words():>8,} "
            f"{chapter.total_tics():>6} "
            f"{chapter.tic_density():>8.2f}"
        )
        lines.append("")
...
def main():
    """Main entry point."""
    print("=" * 60)
    print("SCENE-LEVEL PROSE TIC ANALYZER - The Wheel of Meat")
    print("=" * 60)
    print("")
    ...
    print("Analyzing all scene files...")
    print("-" * 60)
    ...
    print(f"\n✓ Analysis complete:")
    print(f"  - {len(analyses)} chapters processed")
    print(f"  - {total_scenes} scenes analyzed")
    print(f"  - {total_tics} tics identified")
    print("")

    # Save JSON data
    json_output = OUTPUT_DIR / "tic_analysis_scenes.json"
        ...
        json.dump(
            {
                ...
                'chapters': [ch.to_dict() for ch in analyses]
            },
            f,
            indent=2
        )
    ...
    markdown_output = OUTPUT_DIR / "tic_revision_report_scenes.md"
    ...
    summary_output = OUTPUT_DIR / "tic_summary_scenes.txt"
    ...
    print("")
    print("=" * 60)
    print(f"TOTAL TICS FOUND: {total_tics} across {total_scenes} scenes")
    print("=" * 60)
